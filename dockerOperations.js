...
async function checkImageExists(imageName) {
    try {
        const { stdout } = await execAsync('docker image ls --format "{{.Repository}}:{{.Tag}}"');
        ...
        const imageExists = images.some(image => {
            return image === imageName || 
                   ...
                   (imageName.includes(':') === false && image.startsWith(`${imageWithoutTag}:`));
        });
...
function buildDockerImage(workspaceFolder, imageName) {
    return new Promise((resolve, reject) => {
        ...
        const dockerfilePath = path.join(workspaceFolder, '.codeforge', 'Dockerfile');
...
function runDockerCommand(workspaceFolder, imageName, command, options = {}) {
    ...
    if (command) {
        ...
        const commandParts = typeof command === 'string' ? command.split(' ') : command;
