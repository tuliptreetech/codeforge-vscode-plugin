...
suite('CodeForge Extension Test Suite', () => {
    ...
    suite('Command Tests', () => {
        test('Initialize command should create .codeforge directory', async () => {
            ...
            const accessStub = sandbox.stub(fs, 'access').rejects(new Error('Not found'));
            ...
            assert.ok(mkdirStub.calledWith(path.join('/test/workspace', '.codeforge'), { recursive: true }));
            ...
            assert.ok(showInformationMessageStub.calledWith('CodeForge: Successfully initialized .codeforge directory'));
        });
        ...
        test('Initialize command should handle existing directory', async () => {
            ...
            const showWarningMessageStub = sandbox.stub(vscode.window, 'showWarningMessage').resolves('No');
            const workspaceFolderStub = sandbox.stub(vscode.workspace, 'workspaceFolders').value([{
                uri: { fsPath: '/test/workspace' }
            }]);
        ...
        test('Build command should check for Dockerfile existence', async () => {
            ...
            assert.ok(showErrorMessageStub.calledWith('CodeForge: Dockerfile not found. Please run "Initialize CodeForge" first.'));
        });

        test('Commands should handle no workspace folder', async () => {
            // Mock no workspace folder
            const workspaceFolderStub = sandbox.stub(vscode.workspace, 'workspaceFolders').value(undefined);
            const showErrorMessageStub = sandbox.stub(vscode.window, 'showErrorMessage');
            ...
            for (const command of commands) {
                ...
                assert.ok(showErrorMessageStub.calledWith('CodeForge: No workspace folder is open'));
