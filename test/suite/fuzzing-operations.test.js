...
suite('Fuzzing Operations Test Suite', () => {
  ...
  suite('Basic Functionality', () => {
    ...
    test('createFuzzingDirectory should return correct path', async () => {
      ...
      const result = await fuzzingOperations.createFuzzingDirectory(workspacePath);
      assert.strictEqual(result, expectedPath);
    });

    test('handleFuzzingError should show error message', async () => {
      const showErrorMessageStub = sandbox.stub(vscode.window, 'showErrorMessage').resolves('Cancel');
      
      const error = new Error('Test error');
      const result = await fuzzingOperations.handleFuzzingError(error, 'test context', mockOutputChannel);
      
      assert(showErrorMessageStub.calledOnce);
      assert(mockOutputChannel.appendLine.calledWith('[Fuzzing] Fuzzing test context failed: Test error'));
    ...
    test('generateFuzzingSummary should format results correctly', () => {
      const results = {
        ...
        executedFuzzers: 3,
        crashes: [
          { fuzzer: 'test-fuzz', file: '/path/to/crash' }
        ],
        errors: [
          { type: 'build', error: 'Build failed' }
        ]
      };
  ...
  suite('Integration with Docker Operations', () => {
    test('should use generateContainerName from dockerOperations', () => {
      const generateContainerNameStub = sandbox.stub(dockerOperations, 'generateContainerName').returns('test-container');
    ...
    test('should validate module exports', () => {
      ...
      assert.strictEqual(typeof fuzzingOperations.orchestrateFuzzingWorkflow, 'function');
      assert.strictEqual(typeof fuzzingOperations.createFuzzingDirectory, 'function');
      assert.strictEqual(typeof fuzzingOperations.safeFuzzingLog, 'function');
      assert.strictEqual(typeof fuzzingOperations.handleFuzzingError, 'function');
      assert.strictEqual(typeof fuzzingOperations.generateFuzzingSummary, 'function');
    });
  });

  suite('Error Handling', () => {
    test('handleFuzzingError should provide retry option', async () => {
      const showErrorMessageStub = sandbox.stub(vscode.window, 'showErrorMessage').resolves('Retry');
      
      const error = new Error('Retry test');
      const result = await fuzzingOperations.handleFuzzingError(error, 'retry context', mockOutputChannel);
    ...
    test('handleFuzzingError should show output channel when requested', async () => {
      const showErrorMessageStub = sandbox.stub(vscode.window, 'showErrorMessage').resolves('View Output');
      
      const error = new Error('View output test');
      const result = await fuzzingOperations.handleFuzzingError(error, 'view context', mockOutputChannel);
